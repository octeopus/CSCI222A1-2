...
void logSys::membersystem()	//member login system
{
	...
	//if index = 0, go to registration 
	//if index > 0, go to log in page instead. 
...
void logSys::MemLogin()
{
	...
	for(size_t i = 0; i != memVec.f.size(); i++){
		if(inputUser.compare(memVec.f[i].getMemUserName())&& inputPass.compare(memVec.f[i].getMemPassword())){
			ifFound = true;
			...
			do
			{
			    ...
			    switch(MEoption)
			    {
			    	case '1': cout << "Member Profile System" << endl;	//Facilities Management System
			    				//call Member Profile System while passing MemberID into the function
			    			  break;
